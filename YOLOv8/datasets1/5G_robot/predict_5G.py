...
count_frame = 0
model_path = r'C:\Users\Hi1pp\5G_YOLOv8\runs\detect\train13\weights\best.pt'
    ...
    if len(object_queues) >= delay: 
        for i in range(len(object_queues)-delay, len(object_queues)):
            ...
            if (distence >= distance_threshold).any():
                ...
                count = 0
            if count >= delay: #梨原點超過20禎，則將新位置設為原點
                return True
    return False
...
fps = 30
boxcenter_coord = []
init_coord = [] #init_coord[0] = screw_driver boxcenter, init_coord[1]=brush boxcenter
action_brush = True
action_screw = True
...
while True:
    ...
    if ret == True: 
        boxcenter_coord = []
        results = model.predict(frame, save=True, conf=0.75)
        if count_frame >= 20: #超過20楨才開始算物件中心點
            for result in results:
                ...
                for i, box in enumerate(result.boxes):
                    ...
                    if  object_name == 'screw_driver': #要改screw_driver
                ...
                if action_brush :
                    ...
                    if action_brush == False:
                        ...
                        cv2.putText(new_frame, 'bursh', (300,50), cv2.FONT_HERSHEY_COMPLEX, 2, (0,255,0),2) 
                if action_screw :
                    ...
                    if action_screw == False:
                        ...
                        cv2.putText(new_frame, 'screw up', (300,50), cv2.FONT_HERSHEY_COMPLEX, 2, (0,255,0),2) 
