...
model_path = r'C:\Users\Hi1pp\5G_YOLOv8\runs\detect\train11\weights\best.pt'
video_path = r"C:\Users\Hi1pp\5G_YOLOv8\datasets\5G_robot\normal_h22.mp4"
...
out = cv2.VideoWriter('datasets/5G_robot/output.mp4',fourcc, 30.0, size)
#===========================================================#
# 區域框大小設定
#===========================================================#
current_region = None
counting_regions = [
    {
        ...
        "polygon": Polygon([(180, 320), (350, 320), (350, 580), (180, 580)]),  # Polygon points
        # "counts": 0,
        "dragging": False,
     ...
     {
        "name": "YOLOv8 Rectangle Region",
        "polygon": Polygon([(410, 530), (490, 530), (490, 600), (410, 600)]),  # Polygon points
        # "counts": 0,
        "dragging": False,
...
action_queue_brush = []
action_queue_screw = []
count_time = [0,0] #count_time[0]=brush time, count_time[1]=screw driver time
line_thickness = 2
            ...
            if count_frame >= 20: #超過20楨才開始算物件中心點
                for result in results:
                    ...
                    for i, box in enumerate(result.boxes):
                        ...
                        if  object_name == 'screw_driver': 
                            screw_boxcenter_coord.append([center_x, center_y])
                        elif object_name == 'brush':
                            brush_boxcenter_coord.append([center_x, center_y])
                        elif object_name == 'stickers':
                            sticker_x, sticker_y = center_x, center_y
                        cv2.circle(frame, (center_x, center_y), 5,(0,0,255),-1) #畫出每個物件框的中心點

                        # Check if detection inside region
                        # 有東西在這個區域才會進判斷
                        # for region in counting_regions:
                        #     if region["polygon"].contains(Point((sticker_x, sticker_y))):
                        #         cv2.putText(frame, 'Scan', (300,50), cv2.FONT_HERSHEY_COMPLEX, 2, (0,255,0),2)
                        #     elif region['polygon'].contains(Point(sticker_x, sticker_y)):   
                        #         cv2.putText(frame, 'Stickers', (300,50), cv2.FONT_HERSHEY_COMPLEX, 2, (0,255,0),2)
                        if counting_regions[0]["polygon"].contains(Point(sticker_x, sticker_y)):
                            cv2.putText(frame, 'Scan', (300,50), cv2.FONT_HERSHEY_COMPLEX, 2, (0,255,0),2)
                        if counting_regions[1]["polygon"].contains(Point(sticker_x, sticker_y)):
                            cv2.putText(frame, 'Stickers', (300,50), cv2.FONT_HERSHEY_COMPLEX, 2, (0,255,0),2)
                    ...
                    if action_brush :
                        ...
                        if action_brush == False:
                            ...
                            cv2.putText(frame, 'brush', (300,50), cv2.FONT_HERSHEY_COMPLEX, 2, (0,255,0),2) 
                    ...
                    if action_screw :
                        ...
                        if action_screw == False:
                            ...
                            cv2.putText(frame, 'screw up', (300,50), cv2.FONT_HERSHEY_COMPLEX, 2, (0,255,0),2) 
                    cv2.putText(frame, 'Action:', (50,50), cv2.FONT_HERSHEY_COMPLEX, 2, (0,255,0),2) 
                    cv2.putText(frame, 'screw driver:' + str(round(count_time[0],3)) + 's', (50,100), cv2.FONT_HERSHEY_COMPLEX, 1, (255,0,0),2) 
                    cv2.putText(frame, 'brush:' + str(round(count_time[1],3)) + 's', (50,150), cv2.FONT_HERSHEY_COMPLEX, 1, (255,0,0),2) 
